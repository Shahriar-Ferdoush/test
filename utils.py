...
def multimodal_tokenize(batch, processor, device, context_templates=None, hparams=None):
    ...
    for i, loc_prompt in enumerate(loc_prompts):
        if loc_prompt in prompts[i]:  # subject: Factual Editing
            subject_token = processor.tokenizer.encode(
                " " + loc_prompt, add_special_tokens=False
            )
            subject_token1 = processor.tokenizer.encode(
                loc_prompt, add_special_tokens=False
            )
            subject_length = len(subject_token)
            act_mask = torch.zeros_like(
    ...
    if file_type in ["image", "single-image", "multi-image"]:
        multimodal_inputs = processor(
            images=input_images, text=full_prompt, return_tensors="pt", padding=True
        ).to(device, dtype=torch.float32)
    elif file_type == "video":
        multimodal_inputs = processor(
            videos=input_images[0], text=full_prompt, return_tensors="pt", padding=True
        ).to(device, dtype=torch.float32)

    last_prompt_token_loc = (tokens["labels"] == -100).sum(dim=-1)[0]
    last_ans_token_loc = (tokens["labels"] == processor.tokenizer.pad_token_id).sum(
        dim=-1
    )[0]
    ans_token_len = (
        len(tokens["labels"][0]) - last_prompt_token_loc - last_ans_token_loc
    )
