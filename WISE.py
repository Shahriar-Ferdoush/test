...
class WISE(torch.nn.Module):
    ...
    def __call__(self, **kwargs):
        if not self.config.retrieve:
            if (
                hasattr(self.get_adapter_layer(), "editing")
                and not self.get_adapter_layer().editing
            ):
                # final merge
                if (
    ...
    def _cal_activation_loss(
        ...
        for i, act_mk in enumerate(act_mask):
            if act_mk is not None:
                in_scope_dist = euc(
                    original_layer_output[
                        int(i * len_temp) : int((i + 1) * len_temp), ...
                    ],
                    new_weight_layer_output[
                        int(i * len_temp) : int((i + 1) * len_temp), ...
                    ],
                ...
                if i == k - 1:
                    out_scope_dist = euc(
                        original_layer_output[int(i - k) :, ...],
                        new_weight_layer_output[int(i - k) :, ...],
                        config,
                    )
                else:
                    out_scope_dist = euc(
                        original_layer_output[int(i - k) : int(i + 1 - k), ...],
                        new_weight_layer_output[int(i - k) : int(i + 1 - k), ...],
                        config,
                    )
    ...
    def _cal_memory_pos_activation_loss(
        self,
        original_layer_output,
        new_weight_layer_output,
        config=None,
    ...
    def _cal_memory_neg_activation_loss(
        self,
        original_layer_output,
        new_weight_layer_output,
        config=None,
    ...
    def save(self, save_path):
        ...
        if (
            hasattr(self.get_adapter_layer(), "key_id")
            and self.get_adapter_layer().key_id is not None
        ):
    ...
    def load(self, load_path):
        ...
        if saved_data["config"] != self.config:
            print(
                "Warning: The loaded WISE config is different from the original config"
            )
...
class WISEAdapter(torch.nn.Module):
    ...
    def new_weight_forward(self, input: Tensor) -> Tensor:
        return (
            F.linear(input, self.new_weight)
            if self.bias is None
            else torch.addmm(
                self.bias, input.view(-1, input.size(-1)), self.new_weight
            ).view(input.size()[:-1] + (self.layer.nf,))
        )
